...
class SchemaManager:
    ...
    def ensure_fields_parquet(self, obj_name, arrow_schema, concurrency=20):
        print(f"Ensuring fields for {obj_name} (Parquet Schema)...")
        import pyarrow as pa
        
        fields_to_create = []

        for field in arrow_schema:
            col_name = field.name.lower().strip()
            if not col_name: continue
            
            # Truncate
            if len(col_name) > 63:
                print(f"⚠️ Truncated field '{field.name}' -> '{col_name[:63]}'")
            ...
            if pa.types.is_boolean(pa_type):
                ...
                # Heuristic: IDs often come as numbers in some exports, but usually strings.
                # If it ends in _id, treat as Lookup or Text, unless we are sure.
                # But Parquet types are strong. If it says Int, it is Int.
                # EXCEPT: Salesforce IDs are strings.
                # Check heuristic for "_id" fields that might be FKs?
                # Actually, if it is an ID, it should be a String in Salesforce. 
                # If it is numeric in parquet, it's a number.
                type_name = 'Number'
            elif pa.types.is_timestamp(pa_type) or pa.types.is_date(pa_type):
                type_name = 'DateTime'
            else:
                # String / Binary / LargeString
                # Default to LongTextArea for safety unless we check lengths?
                # For schema speed, let's default to LongTextArea to avoid truncation errors.
                # Or Text if it looks like a name?
                # "Name" -> Text.
                lower = col_name.lower()
                if lower == 'name' or lower == 'title' or lower == 'email' or lower == 'phone':
                    type_name = 'Text'
                elif lower.endswith('_id'):
                    type_name = 'Lookup'
                    target = field.name[:-3] # original case suggestion?
                    extra = {"logicalType": "Lookup", "referenceTo": [target]}
                else:
                    type_name = 'LongTextArea'

            # Override sensitive fields
            if 'phone' in col_name or 'mobile' in col_name:
                type_name = 'Text' # Phone
            if 'email' in col_name:
                type_name = 'Email'
            if 'url' in col_name or 'website' in col_name: 
                type_name = 'Url'
            
            fields_to_create.append((col_name, type_name, field.name, extra))
