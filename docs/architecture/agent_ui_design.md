...
### 2.1 The Omnipresent Copilot (Utility Bar)
**Component**: `components/AIAssistant.tsx` (Revamped)
**Location**: Persistent in the `UtilityBar` (bottom or side).
*   **Context-Aware**: When viewing `Account/123`, the Copilot knows it.
*   **Interaction**:
    *   **Chat**: "Enrich this account."
    *   **Magic Fill**: Clicking "Magic Wand" icon on a Form (`MetadataRecordForm`) triggers the agent to draft fields based on the Copilot context.

### 2.2 In-Flow Proposals (Action Injection)
Agents don't just chat; they propose state changes.
**Mechanism**:
...
### 2.3 Escalation Queue (Inbox)
**Component**: `NotificationCenter.tsx` + New `EscalationList` component.
...
## 3. Admin Interaction (The Architect)

### 3.1 Agent Studio (Leveraging `components/studio`)
Admins define Agents just like they define Objects.
*   **Builder UI**:
    *   **Profile**: Name, Avatar, Base Model (Claude/GPT).
    *   **Permissions**: Link to `_System_Profile` (uses existing `UserPermissionSetsSection`).
    *   **Skills**: Drag-and-drop enabled Skills from the Skills Registry.

### 3.2 Skills Monitor
A dashboard to see "What are my agents doing?"
...
*   **No new "AI Portal"**.
*   **Deep Linking**: Agents live in the Utility Bar and Notifications.
